...
func initialize(_skill_name):
	...
	$HBoxContainer/Skill/SkillIcon.texture_normal = load(PlayerStats.skill_textures[skill_name])
	$HBoxContainer/VBoxContainer/Level.text = "Level " + str(PlayerStats.skills[skill_name]["level"])
...
func update_exp_bar():
	var data = PlayerStats.get_skill_ui_exp(skill_name)
...
func update_tags():
	clear_tags()
	for tag in PlayerStats.skill_tags[skill_name]:
